...
void KeyFrame::save(Archive & ar, const unsigned int version) const
{
    ...
    //if (mbBad)
    //return;
    ar & const_cast<uint64_t &> (mnId);
    ar & const_cast<uint64_t &> (mnFrameId);
    ar & const_cast<double &> (mTimeStamp);
    ar & const_cast<int &> (mnGridCols);
    ar & const_cast<int &> (mnGridRows);
    ar & const_cast<float &>  (mfGridElementWidthInv);
    ar & const_cast<float &>  (mfGridElementHeightInv);
    ar & const_cast<uint64_t &> (mnTrackReferenceForFrame);
    ar & const_cast<uint64_t &> (mnFuseTargetForKF);
    ar & const_cast<uint64_t &> (mnBALocalForKF);
    ar & const_cast<uint64_t &> (mnBAFixedForKF);
    ar & const_cast<uint64_t &> (mnLoopQuery);
    ar & const_cast<int &> (mnLoopWords);
    ar & const_cast<float &> (mLoopScore);
    ar & const_cast<uint64_t &> (mnRelocQuery);
    ar & const_cast<int &> (mnRelocWords);
    ar & const_cast<float &> (mRelocScore);
    ar & const_cast<cv::Mat &> (mTcwGBA);
    ar & const_cast<cv::Mat &> (mTcwBefGBA);
    ar & const_cast<uint64_t &> (mnBAGlobalForKF);
...
//    ar & const_cast<std::vector<cv::Point2f> &> (mvQuadPoints);
//    ar & const_cast<std::vector<cv::Point2f> &> (mvSems);
//    ar & const_cast<std::vector<cv::Point2f> &> (mvSemsUn);
//    ar & const_cast<std::vector<cv::Point3f> &> (mvMapSemPoints);
//    ar & const_cast<std::string &> (mImgName);
...
//    ar & const_cast<std::string &> (mImgName);
    ...
    if (has_parent)
    {
        ...
        ar & mparent_KfId_map.map_id;
    }
    else
    {
        mparent_KfId_map.is_valid = false;
        mparent_KfId_map.map_id = 0;
    }
...
template void KeyFrame::save<portable_binary_oarchive>(portable_binary_oarchive &, const unsigned int) const;
template void KeyFrame::load<portable_binary_iarchive>(portable_binary_iarchive &, const unsigned int);
...
void KeyFrame::SetMapPoints(std::vector<MapPoint*> spMapPoints)
{
    ...
    int j = 0, ctr = 0;
    for (std::map<uint64_t,id_map>::iterator it = mmMapPoints_nId.begin();
...
void KeyFrame::SetSpanningTree(std::vector<KeyFrame*> vpKeyFrames)
{
    ...
    j = 0; ctr = 0;
    is_valid = false;
    for (std::map<uint64_t,id_map>::iterator it = mmChildrens_nId.begin();
    ...
    j = 0; ctr = 0;
    is_valid = false;
    for (std::map<uint64_t,id_map>::iterator it = mmLoopEdges_nId.begin();
...
void KeyFrame::SetGridParams(std::vector<KeyFrame*> vpKeyFrames)
{
    ...
    for (map<uint64_t, int>::iterator it = mConnectedKeyFrameWeights_nId.begin();
    ...
    j = 0;
    for (std::map<uint64_t,id_map>::iterator it = mvpOrderedConnectedKeyFrames_nId.begin();
...
void KeyFrame::SetKFDB(KeyFrameDatabase *pKFDB){
    mpKeyFrameDB = pKFDB;
}

KeyFrameDatabase* KeyFrame::GetKFDB(){
    return mpKeyFrameDB;
}
...
void KeyFrame::SetConnectedKeyFrameWeights(std::map<KeyFrame*,int> connectedKeyFrameWeights){
    mConnectedKeyFrameWeights = connectedKeyFrameWeights;
}

std::map<KeyFrame*,int> KeyFrame::GetConnectedKeyFrameWeights(){
    return mConnectedKeyFrameWeights;
}

void KeyFrame::SetOrderedConnectedKeyFrames(std::vector<KeyFrame*> vpOrderedConnectedKeyFrames){
    mvpOrderedConnectedKeyFrames = vpOrderedConnectedKeyFrames;
}

std::vector<KeyFrame*> KeyFrame::GetOrderedConnectedKeyFrames(){
