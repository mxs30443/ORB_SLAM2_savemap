...
void MapPoint::save(Archive & ar, const unsigned int version) const
{
    ...
    //cout << "[" << mnId << "]" ;
    ar & nNextId;
    ar & const_cast<int64_t &> (mnFirstKFid);
    ar & const_cast<int64_t &> (mnFirstFrame);
    ...
    ar & const_cast<uint64_t &> (mnLastFrameSeen);
    ar & const_cast<uint64_t &> (mnBALocalForKF);
    ar & const_cast<uint64_t &> (mnFuseCandidateForKF);
    ar & const_cast<uint64_t &> (mnLoopPointForKF);
    ar & const_cast<uint64_t &> (mnCorrectedByKF);
    ar & const_cast<uint64_t &> (mnCorrectedReference);
...
//    ar & const_cast<std::vector<cv::Mat> &> (mvDescriptor);
...
template void MapPoint::save<portable_binary_oarchive>(portable_binary_oarchive &, const unsigned int) const;
template void MapPoint::load<portable_binary_iarchive>(portable_binary_iarchive &, const unsigned int);

void MapPoint::SetObservations(std::vector<KeyFrame*> spKeyFrames)
{

    uint64_t id, kfRef_id; size_t size;
    ...
    for (map<uint64_t, size_t>::iterator it = mObservations_nId.begin(); it != mObservations_nId.end(); j++,++it) {
        id = it->first;
        size = it->second;
        {
