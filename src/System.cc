...
int ReadBinaryFile(const char* filename, char* & buf)
{
	int len = 0;
	buf = NULL;
	FILE* fd = (filename && filename[0]) ? fopen(filename, "rb") : NULL;
	if (fd != NULL)
	{
		fseek(fd, 0, SEEK_END);
		len = ftell(fd);
...
System::System(const string &strVocFile, const string &strSettingsFile,const string &strMapFile, const eSensor sensor,
               const bool bUseViewer,bool bloadmap ,bool bkeepmap ):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),
                mbActivateLocalizationMode(bkeepmap),mbDeactivateLocalizationMode(false)
{
    // Output welcome message
    cout << endl <<
    "ORB-SLAM2 Copyright (C) 2014-2016 Raul Mur-Artal, University of Zaragoza." << endl <<
...
void System::loadMapFromByteArray(const void* mapAdd, size_t size)
{
    if (mapAdd && size > 0)
    {
        boost::asio::streambuf b;
        boost::asio::mutable_buffer buf = b.prepare(size);
        memcpy(boost::asio::detail::buffer_cast_helper(buf), mapAdd, size);
        b.commit(size);
        std::istream is(&b);
...
int System::saveMapToByteArray(char * &p)
{
    boost::asio::streambuf b;
    std::ostream os(&b);
    {
    ...
    const boost::asio::const_buffer buf = b.data();
    int n = boost::asio::detail::buffer_size_helper(buf);
    if (n > 0)
    {
        p = (char*)malloc(n);
        memcpy(p, boost::asio::detail::buffer_cast_helper(buf), n);
        b.consume(n);
    }
    else
    {
    ...
    cout << "The size of saved map: " << n << endl;
    LOG("The size of saved map: %d bytes", n);
#endif
    return n;
}
...
{
	char* bufMap = NULL;
	int lenMap = ReadBinaryFile(filename.c_str(), bufMap);
    ASSERT(lenMap > 0 && bufMap,"Open map failed!");
    loadMapFromByteArray(bufMap, lenMap);
	if (bufMap)
	{
		free(bufMap);
	}
